...
* 🎬 **Google Vertex AI Veo 3.1 & Imagen 3 Engine**: Powered by Google Cloud's `aiplatform.googleapis.com` via Service Account OAuth 2.0 authentication (`gcp_key.json`), ensuring **100% coverage by $300 GCP Cloud Credits** with **₹0.00** out-of-pocket cost.
* ☀️🌙 **Full Responsive Light & Dark Mode Studio Architecture**: Complete theme-adaptive UI system across Rukhi Film Studio tabs (`SeriesBibleTab`, `CharacterVaultTab`, `LocationCatalogTab`, `DirectorTimelineTab`, `SceneGalleryTab`), Home Page, Dashboard, Pricing & Guide Modals, and Site Footer.
* 💎 **Professional Lucide Vector Icon Suite**: Replaced all raw emojis across AI Director forms, Scene Timeline cards, user profile dropdowns, and modal dialogs with high-visibility vector icons (`Film`, `Download`, `Trash2`, `Sparkles`, `Crown`, `Zap`, `Headphones`, `MessageSquare`).
* 📊 **Production Cost Ledger & Per-User Telemetry System**: Production-grade cost accounting engine ([`productionLedgerService.js`](file:///home/anji/Documents/auto_captions/backend/src/services/studio/productionLedgerService.js) & [`pricingConfig.js`](file:///home/anji/Documents/auto_captions/backend/src/config/pricingConfig.js)) that tracks real token counts (Gemini 2.5 Flash / 1.5 Pro), image renders (Imagen 3), video seconds (Veo 3), STT minutes (Deepgram/Whisper), dubbing minutes (Demucs), voice cloning samples, and storage, auto-persisting exact USD & INR costs per user into PostgreSQL (`studio_generation_costs`).
* 🎬 **Rukhi Studio Hollywood Director Pipeline**: 7-Department AI production orchestrator ([`directorOrchestratorService.js`](file:///home/anji/Documents/auto_captions/backend/src/services/studio/directorOrchestratorService.js)) compiling screenplay manifests, character visual continuity locking, set location specs, 180° film grammar preflight checks, and Pinecone vector RAG memory indexing.

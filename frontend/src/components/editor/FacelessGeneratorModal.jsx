...
import { X, Sparkles, Video, Wand2, Globe, Volume2, Check, Loader2, Play, UserCheck, Layers, Clock, Zap, Monitor, Smartphone, Square } from 'lucide-react';
...
const SAMPLE_TOPICS = [
  { 
    label: '🎬 Artisan Watchmaker Dusk', 
    prompt: 'A cinematic, continuous 30-second tracking shot inside a cluttered artisan watchmaker workshop at dusk. Micro-tweezers inserting gear with tiny sparks, camera tracking across oak floor, steam rising from kettle, opening velvet curtains to rain-streaked window and neon street.',
    style: 'CINEMATIC',
  },
  { 
...
const CHARACTER_STYLES = [
  { id: 'CINEMATIC', label: '🎬 Cinematic IMAX', subtitle: 'Photorealistic, 8k 65mm lens, 24fps' },
...
const ASPECT_RATIOS = [
  { id: '9:16', label: '📱 9:16 Vertical', subtitle: 'Reels, Shorts, TikTok', icon: Smartphone, requiresPro: false },
  { id: '16:9', label: '📺 16:9 Landscape', subtitle: 'YouTube Main (Pro Plan)', icon: Monitor, requiresPro: true },
  { id: '1:1', label: '🔳 1:1 Square', subtitle: 'Instagram & LinkedIn (Pro Plan)', icon: Square, requiresPro: true },
];
...
export default function FacelessGeneratorModal({ isOpen, onClose, onProjectCreated, onSuccess }) {
  ...
  const [characterStyle, setCharacterStyle] = useState('CINEMATIC');
  ...
  const isProOrStudio = ['pro', 'dubbing_studio', 'admin'].includes(userPlan);

  useEffect(() => {
    if (isOpen) {
      setPrompt('');
      setCharacterStyle('CINEMATIC');
  ...
  const handleSelectAspectRatio = (ratioObj) => {
    if (ratioObj.requiresPro && !isProOrStudio) {
      toast.error('16:9 Landscape and 1:1 Square formats require Pro Plan (₹299/mo)');
      setShowPricingModal(true);
      return;
    }
    setAspectRatio(ratioObj.id);
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      toast.success('🎉 AI Video Reel Generated Successfully!');
      ...
      toast.error(err.response?.data?.error || err.message || 'Failed to generate AI Reel');
    } finally {
      setIsGenerating(false);
    }
  };
        ...
        <div className="relative w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden rounded-3xl bg-slate-900 border border-slate-800 shadow-2xl">
          ...
          <div className="flex-shrink-0 flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-950/80">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h2 className="text-lg font-bold text-white tracking-wide">✨ AI Video Reel Creator</h2>
                <p className="text-xs text-slate-400">Veo 3.1 • Consistent Character • Multi-Format Engine</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="flex-1 p-5 sm:p-6 space-y-5 overflow-y-auto custom-scrollbar">
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-300 uppercase tracking-wider flex items-center justify-between">
                <span>1. What is your video topic / prompt?</span>
                <span className="text-[10px] text-cyan-400 font-normal">AI handles script & Veo visuals</span>
              </label>
              <textarea
                rows={3}
                value={prompt}
                onChange={(e) => setPrompt(e.target.value)}
                placeholder="e.g. A cinematic tracking shot inside an artisan watchmaker workshop at dusk..."
            ...
            {/* Quick Suggestions */}
            <div className="space-y-2">
              <span className="text-xs text-slate-400 font-medium">💡 Quick Master Sample Ideas:</span>
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-300 uppercase tracking-wider flex items-center justify-between">
                <span>2. Choose Aspect Ratio Format</span>
                <span className="text-[10px] text-amber-400 font-normal">16:9 & 1:1 require Pro Plan (₹299)</span>
              </label>
                ...
                {ASPECT_RATIOS.map((ratio) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-[10px] text-slate-400 mt-1 truncate">{ratio.subtitle}</div>
                    </button>
                  );
                })}
              </div>
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-300 uppercase tracking-wider flex items-center space-x-2">
                ...
                <span>3. Choose Visual Style & Character Anchor</span>
              </label>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <label className="text-xs font-semibold text-slate-300 uppercase tracking-wider flex items-center space-x-2">
                  ...
                  <span>4. Language & Narration</span>
                </label>
                <div className="space-y-2">
            ...
            >
              {isGenerating ? (
                ...
                <>
                  ...
                  <span>✨ Generate AI Reel ({aspectRatio})</span>
                </>
              )}
            </button>
          </div>
